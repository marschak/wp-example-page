 ...
 */
function alphawell_compile_scss() {
  $compiler = new Compiler();
  $compiler->setSourceMap(Compiler::SOURCE_MAP_FILE);

  $compiler->setOutputStyle(\ScssPhp\ScssPhp\OutputStyle::COMPRESSED);

  if (alphawell_child_has_scss() && is_child_theme()) {
  ...
  if (is_child_theme() && alphawell_child_has_scss()) {
    $compiler->addImportPath(get_template_directory() . '/scss/');
  }

  $last_modified = alphawell_get_last_modified_scss($theme_directory);
  $stored_modified = get_theme_mod('alphawell_scss_modified_timestamp', 0);
  ...
  try {
    if ($last_modified > $stored_modified || !file_exists($css_file) || $is_environment_dev) {
      ...
      set_theme_mod('alphawell_scss_modified_timestamp', $last_modified);
    }
  } catch (Exception $e) {
    wp_die('<b>alphawell SCSS Compiler - Caught exception:</b><br><br> ' . $e->getMessage());
  }
}
 ...
 */
function alphawell_get_last_modified_scss($theme_directory) {
 ...
 */
function alphawell_child_has_scss() {
