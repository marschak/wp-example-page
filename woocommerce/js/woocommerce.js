...
$('.woocommerce-product-rating').click(function(e) {
    $('#reviews').collapse('show');       
    setTimeout(() => {
  $(window).scrollTop($('#reviews').offset().top - 150); 
}, 300)
  //   $(window).scrollTop($('#reviews').offset().top); 
    //  var nav = $(".woocommerce-product-rating");
   // nav.animate({scrollLeft: $('#reviews').offset().left}, '500');
});
}); // jQuery End
